...
<main>
  <div class="p-6 text-4xl font-mono font-bold flex items-center justify-between bg-base-200 sticky top-0">
    ...
    <div class="flex items-center gap-2">
      <svg xmlns="http://www.w3.org/2000/svg" width="44" height="44" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-user-icon lucide-user"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
      {members.length}
    </div>
  </div>
  <div class="p-8 flex flex-col gap-6">
    {#each members as member}
      <div class="flex items-center gap-6">
        <img src={member.displayAvatarURL} alt={member.id} class="h-24 w-24 rounded-full" />
        <div class="break-all">
          <div class="text-5xl">{member.displayName}</div>
          <div class="text-2xl">@{member.username}</div>
        </div>
